    ...
    $.fn.seatCharts = function (setup) {
        ...
        var fn = this,
			...
			settings = {
			    ...
			    contextmenu: function () { return this.style(); },
			    focus: function () {
			...
			seat = (function (seatCharts, seatChartsSettings) {
			    return function (setup) {
			        ...
			        (function (seatSettings, character, seat) {
			            ...
			            $.each(['click', 'focus', 'blur', 'contextmenu'], function (index, callback) {
        ...
        settings.naming.columns = settings.naming.columns || (function (length) {
            ...
            for (var i = 0; i < settings.map.length; i++) {
                rowLength = settings.map[i].split('').length;
                if (rowLength > length)
                    length = rowLength;
            }
            var columns = [];
            ...
            return columns;
        })(settings.map.length > 0 ? settings.map[0].split('').length : 0);
            ...
            $.each(characters.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) {
                ...
                $row.append(character != '_' ?
					...
					(function (naming) {
					    ...
					    var id = overrideId ? overrideId : naming.getId(character, naming.rows[row], naming.columns[column], naming.text);
					    seats[id] = new seat({
					        id: id,
					        label: overrideLabel ?
